...
const userEmail = DEFAULT_USER.email;
...
const DatabaseProvider = ({ children }) => {
	...
	const [userJobsClosed, setUserJobsClosed] = useState([]);
	...
	async function fetchUserJobsClosed() {
		if (userEmail) {
			const { data } = await supabase
				.from('jobs')
				.select('*')
			...
			if (data) {
				setUserJobsClosed(data);
				// console.log('userJobsClosed are', data);
			}
		}
	}
